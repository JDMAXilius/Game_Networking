...
class NetworkMessage
{
	...
	private:
		...
		int rawRead(byte* data, int length, bool partialOkay);
		void rawWrite(const byte* data, int length);

		// Robust functions for reading / writing raw data to the buffer.
		int rawRead(byte* data, int length, int delimiter, bool partialOkay);
		void rawWrite(const byte* data, int length, int delimiter);
};
...
inline int NetworkMessage::rawRead(byte* data, int length, bool partialOkay)
{
	return rawRead(data, length, -1, partialOkay);
}

inline void NetworkMessage::rawWrite(const byte* data, int length)
{
