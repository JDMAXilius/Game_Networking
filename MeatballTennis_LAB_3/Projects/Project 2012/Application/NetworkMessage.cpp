...
{
	return rawRead((byte*) data+offset, length, true);
}

int NetworkMessage::readString(char* data, int length)
{
	return rawRead((byte*) data, length, 0, true);
}

byte NetworkMessage::readByte()
{
	char result;
	rawRead((byte*) &result, 1, false);
	return result;
}
...
{
	rawWrite((byte*) data+offset, length);
}

void NetworkMessage::writeString(const char *data)
{
	rawWrite((byte*) data, (int) strlen(data) + 1, 0);
}

void NetworkMessage::writeString(const char *data, int length)
{
	rawWrite((byte*) data, length, 0);
}

void NetworkMessage::writeByte(byte output)
{
	rawWrite((byte*) &output, 1);
}

void NetworkMessage::writeShort(short output)
{
	output = htons(output);
	rawWrite((byte*) &output, 2);
}

void NetworkMessage::writeInt(int output)
{
	output = htonl(output);
	rawWrite((byte*) &output, 4);
}
...
int NetworkMessage::rawRead(byte* data, int length, int delimiter, bool partialOkay)
{
	if (type == _OUTPUT)
...
void NetworkMessage::rawWrite(const byte* data, int length, int delimiter)
{
	if (type == _INPUT)
